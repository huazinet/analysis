            ...
            <video
              ...
              onError={(e) => {
                ...
                if (!video.hasAttribute('crossorigin')) {
                  ...
                  video.poster = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg>';
                  setError('视频加载失败，请检查链接或稍后重试');
                }
              }}
            />
          </div>
        ...
        {(data.images || data.imgurl) && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(data.images || data.imgurl || []).map((img: string, index: number) => (
              <div key={img} className="group relative bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <img
                  src={img}
                  alt={`图片 ${index + 1}`}
